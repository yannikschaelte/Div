function [varargout] = nllh_rafmekerk_standard(theta, D, shape)
    ...
    if (nargout == 1)
        amiOptions.sensi = 0;
        for j = 1 : 3
            sol = simulate_rafmekerk_gennormal_standard(D(j).t, theta, [D(j).k(:); shape; gamma(1/shape)], D(j), amiOptions);
            if sol.status < 0
                llh = -inf;
            end
            llh = llh + sol.llh;
